...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  private:
    ...
    // flat detId per flatIdx (optional; kept for parity)
  ...
  void Phase2RawToClusterProducer::produce(device::Event& iEvent, device::EventSetup const&) {
    ...
    launchUnpacker(
      ...
      innerDetIdDevice_,   // uint32_t
      outerDetIdDevice_,   // uint32_t
      devClusterProp.view(),
      globalCounter.data()
    );
    ...
    // (Optionally read counter on host for monitoring if you want)
