...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
	...
	Phase2RawToClusterProducer::Phase2RawToClusterProducer(const edm::ParameterSet& iConfig)
		...
		myqueue(devAcc),
		// Make the Global host and device buffer for each Module Type with the given size = dtcId * number of slinks * number of channels 
	...
	void Phase2RawToClusterProducer::produce(
		...
		*/
		...
		//TODO:: Check this section if needed to move back to reserve from resize 
...
alpaka::memcpy(queue, hostClusterPropSoA.buffer(), devClusterProp.const_buffer());
alpaka::wait(queue);

// Emplace the SoA collection into the event
iEvent.emplace(outputToken_, std::move(hostClusterPropSoA));
		// TODO:: copy the unpacked value to the ClusterPropSoA 
		// iEvent.emplace 
