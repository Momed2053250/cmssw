...
## Run the EDAnalyzer on the original (CPU) cluster collection
output_original='clusterNtuple_original_TTBar.root'
cmsRun Phase2TrackerDumpClusters_cpu_cfg.py inputFiles_clear inputFiles=$filename outputFile=$output_original onUnpacked=false

## Run the EDAnalyzer on the unpacked (GPU) cluster collection
output_recluster='clusterNtuple_reclustered_TTBar.root'
filename='file:raw2clusters.root'
cmsRun Phase2TrackerDumpClusters_gpu_cfg.py inputFiles_clear inputFiles=$filename outputFile=$output_recluster onUnpacked=true
...
## Run the validation script, selecting clusters from a specific DTC
